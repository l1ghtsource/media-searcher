...
Изначально *OCRModel* считывал текст с фреймов последовательно, а в *MetaModel* происходило распараллеливание *CLIPModel*, *OCRModel* и *WhisperModel*. Однако, когда мы начали проводить замеры скорости на CPU, было обнаружено, что если распараллелить именно процесс считывания текста с фреймов в *OCRModel*, то скорость значительно возрастет. То есть сначала на все возможные ресурсы параллелится *OCRModel*, а когда считываение текста завершится уже *CLIPModel* и *WhisperModel* распределяют ресурсы между собой. Это ускорило инференс на CPU с одной минуты до 30 секунд!
...
5) Собственный переводчик, зафайнтюненный на корпусе из популярных в Yappy слов (верно интерпретируем слэнг-запросы)
6) Мультиязычность (решение не зависит ни от языка видео, ни от языка поискового запроса)
...
9) Скорость построения новых эмбеддингов и занесения их в *Clickhouse*-хранилище, на GPU за 5-25с (в зависимости от длины видео) будут получены транскрибация, текст с видео, построены три эмбеддинга и занесены в БД
